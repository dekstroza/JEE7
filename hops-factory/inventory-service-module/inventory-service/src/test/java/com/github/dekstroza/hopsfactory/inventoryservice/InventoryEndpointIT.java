...
public class InventoryEndpointIT {
    ...
    public void createValidInventory() {
        ...
        ExtractableResponse response = given().body(inventory).contentType(JSON).when().post(INVENTORY_SERVICE_URL).then().assertThat().statusCode(
                   201).and().contentType(JSON).extract();
